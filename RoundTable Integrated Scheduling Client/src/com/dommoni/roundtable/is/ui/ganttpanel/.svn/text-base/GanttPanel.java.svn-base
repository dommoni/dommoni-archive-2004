   ...
   implements MouseListener, MouseMotionListener, MouseWheelListener
{
   ...
   // images used for gradients
	...
	private Image imgLBlueGrad = null; 
	private Image imgDBlueGrad = null; 
   ...
   public static final int ZOOM_MIN = VMODE_WEEK;
   public static final int ZOOM_MAX = VMODE_6MONTH;
   ...
   public static final int taskLineHeight = 19;
	...
	public GanttPanel(CursorController _cursorController, Map images)//ControlCollection cc, CursorController _cursorController)
	{
      ...
      mo6 *= 30;
      mo6 *= 6;
      long time = clEndTime.getTimeInMillis() + mo6;
      ...
      setBorder(new EtchedBorder());//new ShadowBorder());
	}
...
/*			// Draw Task-Title Column
   ...
   {
      task.setXCoord(2 + taskColumnOffset);
      task.setDuration(0.5f);
      task.setWidth(getTaskPixelWidthForDuration(0.5f));
      
      Calendar clToday = (Calendar)clViewTime.clone();
      task.setStartDate(clToday);
      
      clToday.set(Calendar.DAY_OF_YEAR, clToday.get(Calendar.DAY_OF_YEAR) + 1);
      task.setEndDate((Calendar)clToday.clone()); 
      
      remapXCoordForTask(task);
   ...
   /**
    * Loads existing tasks retrieved from a data-source
    *   i.e. RoundTable Server.
    *   
    */
   public void loadTask(ScheduleTask task)
   {
      if (task != null)
      {
         // init gantt params
         task.setWidth(getTaskPixelWidthForDuration(task.getDuration()));
         task.setXCoord(makeXCoord(task));
         
         task.setHighlighted(false);
         task.setSelected(false);
         task.setPinched(false);
         task.setWasDragged(false);
   ...
   public void removeTask(GanttTask task) throws GanttException
   {
      try
      {
         List lines = this.lineModel.getLines();
         ...
         for (int i = 0; i < lines.size(); i++)
         {
            ...
            if (l != null)
            {
               if (l.task.equals(task))
               {   
                  taskFound = true;
                  lineModel.removeLine(l);
                  
                  // reset task-ycoords.
                  List newLines = lineModel.getLines();
                  ...
                  TaskActionEvent evt = 
                     new TaskActionEvent(task, TaskActionEvent.TASK_DELETED);
                  fireTaskActionEvent(evt);
                  
                  paint(getGraphics());
               }
            }
            
            // once we find the task we need to exit... 
            // should probably use a while in the main loop...
            if (taskFound) 
               break;
         }
      }
   ...
   public Schedule extractCurrentSchedule()
   {
      List lines = this.lineModel.getLines();
      
      Schedule exSched = this.currentSchedule;
      List tasks = new Vector();
      for (int i = 0; i < lines.size(); i++)
      {
         Line line = (Line)lines.get(i);
         ScheduleItem si = (ScheduleItem)line.task;
         si.setOrdinal(i);
         tasks.add(si);
      }
      exSched.setScheduleItems((ScheduleItem[])tasks.toArray(new ScheduleItem[tasks.size()]));
   ...
   public void remapXCoords()
   {
      ...
      for (int i = 0; i < lines.size(); i++)
      {
         ...
         if (task != null)
         {
            ...
            int newX = pxDays * daysFromStart + taskColumnOffset;
            task.setXCoord(newX);
         }
      }
   }
   ...
   /**
    * Creates an X-Coordinate for a single task given the current 
    * time/scroll structure.
    * 
    * @param task
    * @return
    */
   private int makeXCoord(GanttTask task)
   {
      if (task != null)
      {
         //long daysInTimeModelMillis = 
         ...
         int pxDays = getTaskPixelWidthForDuration(1);
         long minOp = (task.getStartDate().getTimeInMillis() - 
               clBeginTime.getTimeInMillis());
         long daysFromStart = 
           ((task.getStartDate().getTimeInMillis() - 
                 clBeginTime.getTimeInMillis()) / (24 * 60 * 60 * 1000));
         int x = (pxDays * (int)daysFromStart) + taskColumnOffset;
   ...
   public void remapXCoordForTask(GanttTask task)
   {
      if (task != null)
      {
         // find days from beginning, multiply that accordingly.
...
//                  clBeginTime.getTimeInMillis()) / (24 * 60 * 60 * 1000));
//         
//         int x = pxDays * daysFromStart + taskColumnOffset;
//         float rm = task.getXCoord() % x; 
//         System.out.println("remapXCoordForTask.rm: " + rm);
//         System.out.println("Task.X WAS:  " + task.getXCoord());
//         task.setXCoord(pxDays * daysFromStart + taskColumnOffset + getDraggingXOffset());
//         System.out.println("Task.X IS NOW:  " + task.getXCoord());
         
         
         ////////////////////////////////
         int pxDay = getTaskPixelWidthForDuration(1);
         long lDays = clViewTime.getTimeInMillis() - clBeginTime.getTimeInMillis();
         int iDays = (int)(lDays / (24 * 60 * 60 * 1000));
         int pxDaysUnseen = pxDay * iDays;
         task.setXCoord(pxDaysUnseen + task.getXCoord());
      }
   }
   
   private int getXCoordForStartDate()
   {
/*      int pxDays = getTaskPixelWidthForDuration(1);
      int daysFromStart = 
         (int)((task.getStartDate().getTimeInMillis() - 
      ...
      return (pxDays * daysFromStart + taskColumnOffset);
*/      
      return 0;
   }
    ...
    */
	public void updateTaskFromGanttChange(GanttTask task)
	{
      ...
      int loc = 0; 
		...
		int dist = task.getXCoord() - taskColumnOffset - hScrollVal; 
      loc = dist / getTaskPixelWidthForDuration(1);//spacing;
   ...
   public void mousePressed(MouseEvent arg) 
   {
      ...
      if(t != null)
      {
         this.setDraggingXOffset(pt.x - (t.getXCoord() - hScrollVal));
         this.setDraggingYOffset(pt.y - (t.getYCoord() - vScrollVal));
      }
   ...
   private GanttTask getTaskForPoint(Point pt)
   {
      ...
      while(tasks.hasNext())
      {
         GanttTask t = (GanttTask)tasks.next();
         if(
            (pt.x > t.getXCoord() - hScrollVal && 
                  pt.x < t.getXCoord() - hScrollVal + t.getWidth() &&
            pt.y > t.getYCoord() - vScrollVal && 
            pt.y < t.getYCoord() - vScrollVal + GanttPanel.taskHeight) 
            ||
            (pt.x > t.getXCoord() - hScrollVal&& 
             pt.x < t.getXCoord() + this.getTaskPixelWidthForDuration(t.getDuration()) - hScrollVal &&
             pt.y > t.getYCoord() - vScrollVal && 
             pt.y < t.getYCoord() + GanttPanel.taskHeight - vScrollVal)
         )
         {
            return t;
         }
   ...
   public void mouseDragged(MouseEvent arg) 
   {
      ...
      while(tasks.hasNext())
      {
         ...
         if(t.getHighlighted())
         {  
            int newx = makeXCoord(t);
            t.setXCoord(pt.x - this.getDraggingXOffset() + hScrollVal);
            ...
            //remapXCoordForTask(t);
            
            //
            // Do AutoScrolling...
            //
            if (++autoScrollHitCount >= 3)
            {
               if ((t.getXCoord() + (t.getWidth() / 2) - hScrollVal) < taskColumnOffset)
               {
                  if (horizontalScrollBar.getValue() == 0)
                  {
                     // need to add -neg dates to the model.
                  }
                  horizontalScrollBar.setValue(horizontalScrollBar.getValue() - 1);
               }
               else if ((t.getXCoord() + (t.getWidth() / 2) - hScrollVal) > verticalScrollBar.getX())
               {
                  horizontalScrollBar.setValue(horizontalScrollBar.getValue() + 1);
               }
               autoScrollHitCount = 0;
            }
         }
         
         if(t.getPinched())
         {
            int width = pt.x - (t.getXCoord() - hScrollVal);
   ...
   public void mouseMoved(MouseEvent arg) {
      ...
      while(tasks.hasNext())
      {
         ...
         if(pt.x > ((t.getXCoord() + t.getWidth() - 1) - hScrollVal) && 
               pt.x < ((t.getXCoord() + t.getWidth() + 2) - hScrollVal) &&
               pt.y > t.getYCoord() && 
   ...
   //--------------------------------------------------------------------------
   // ShadowBorder - A custom border that has a shadow 
   // on the right and lower sides.
