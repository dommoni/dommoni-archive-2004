 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class MeetingNotificationProcessor extends NotificationProcessor
{
   public MeetingNotificationProcessor(Event event, Notification notifications[])
   {
   ...
   public void run()
   {
      try
      {
         ...
         message.append("Meeting Notification:\n\n");
         ...
         Meeting meeting;
         if(event.getObject() instanceof Meeting)
            meeting = (Meeting)event.getObject();
         else
            return;
   
         SimpleDateFormat sdf = new SimpleDateFormat("EEE, MMMMM d yyyy, hh:mm aaa");
         message.append("meeting " + meeting.getName() + " on " + sdf.format(Calendar.getInstance().getTime()));
...
/*
 *
 * $Log: MeetingNotificationProcessor.java,v $
 * Revision 1.3  2005/10/17 05:20:02  brian
 * Fixed typo.
 *
